...
$(function () {
  ...
  function getStorage () {
    if ((typeof (Storage) !== 'undefined') && (last = localStorage.getItem ('goal_tag_map')) && (last = JSON.parse (last)))
      return last;
    else
      return;
  }
  function setStorage () {
    if (typeof (Storage) !== 'undefined') {
      localStorage.setItem ('goal_tag_map', JSON.stringify ({
        lat: _map.center.lat (),
        lng: _map.center.lng (),
        zoom: _map.zoom
      }));
  ...
  function getGoals () {
    ...
    _getPicturesTimer = setTimeout (function () {
      ...
      .done (function (result) {
        if (result.status) {
          var markers = result.goal_tags.map (function (t) {
